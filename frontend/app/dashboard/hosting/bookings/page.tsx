...
export default async function HostBookingsPage() {
  ...
  const allBookings = propertiesWithBookings.flatMap((p: any) =>
    p.bookings.map((b: any) => ({ ...b, property: p }))
  );

  const now = new Date();

  const pendingBookings = allBookings.filter(
    (b: any) => b.status === "PENDING" && b.checkOut > now
  );

  const confirmedBookings = allBookings.filter(
    (b: any) => b.status === "CONFIRMED" && b.checkOut > now
  );
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] border border-black/10 dark:border-white/10 p-6 rounded-2xl flex flex-col md:flex-row items-center gap-6 shadow-sm">
      ...
      <div className="flex-1 w-full">
        <h3 className="text-lg font-bold font-sora mb-1 text-promaroc-green">
          {booking.property?.title}
        </h3>

        <div className="flex items-center gap-4 text-sm font-medium text-black/60 dark:text-white/60 mb-4">
          <span className="flex items-center gap-1">
            <CalendarIcon className="w-4 h-4" /> {formatDate(booking.checkIn)} -{" "}
            {formatDate(booking.checkOut)}
          </span>
          <span className="flex items-center gap-1">
            <Users className="w-4 h-4" /> {booking.guestsCount} Guests
          </span>
        </div>
