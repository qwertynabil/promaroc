  ...
  return (
    <div className="text-promaroc-black dark:text-promaroc-white animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-sora font-bold mb-2">Host Dashboard</h1>
          <p className="text-black/60 dark:text-white/60 font-medium">
            Welcome back, {session.user.name?.split(" ")[0]}. Here is what&apos;s
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 p-6 rounded-3xl shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 bg-promaroc-green/10 text-promaroc-green rounded-xl">
              <DollarSign className="w-5 h-5" />
            </div>
            <div className="text-black/50 dark:text-white/50 font-bold uppercase tracking-wider text-xs">
              Total Earnings
            </div>
          </div>
      ...
      {properties.length === 0 ? (
        <div className="bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 p-12 rounded-3xl text-center shadow-sm">
          ...
          <p className="text-black/50 dark:text-white/50 mb-6">
            List your first property to start welcoming guests and earning
            revenue.
          </p>
          <Link
            href="/dashboard/hosting/properties/new"
            className="text-promaroc-green font-bold hover:underline"
          >
        ...
        <div className="grid grid-cols-1 gap-4">
          {properties.map((property: any) => (
            <div
              key={property.id}
              className="bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 p-4 rounded-2xl flex flex-col md:flex-row items-center gap-6 shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div className="flex-1 w-full">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold tracking-wider uppercase ${
                      property.status === "APPROVED"
                        ? "bg-promaroc-green/10 text-promaroc-green border border-promaroc-green/20"
                ...
                <div className="flex items-center gap-6 mt-4 pt-4 border-t border-black/5 dark:border-white/10 text-sm font-medium text-black/70 dark:text-white/70">
                  ...
                  <div>
                    <span className="font-bold text-promaroc-black dark:text-promaroc-white">
