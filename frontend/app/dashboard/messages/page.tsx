...
export default async function MessagesPage(props: Props) {
  ...
  const activeChatId = Array.isArray(searchParams.chat)
    ? searchParams.chat[0]
    : searchParams.chat;

  const newChatPropertyId = Array.isArray(searchParams.new_chat)
    ? searchParams.new_chat[0]
    : searchParams.new_chat;

  const currentUserId = session.user.id;

  const conversations = await prisma.conversation.findMany({
    where: {
      OR: [{ guestId: currentUserId }, { hostId: currentUserId }],
    },
    include: {
      property: true,
      guest: true,
  ...
  if (activeChatId) {
    ...
    newChatProperty = await prisma.property.findUnique({
      where: { id: newChatPropertyId },
    });
          ...
          {conversations.length === 0 ? (
            ...
            <div className="flex flex-col">
              {conversations.map((conv: any) => {
                const otherPerson =
                  conv.guestId === currentUserId ? conv.host : conv.guest;
                ...
                return (
                  <Link
                    ...
                    className={`p-4 border-b border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors flex items-center gap-4 ${
                      isActive
                        ? "bg-black/5 dark:bg-white/5 border-l-4 border-l-promaroc-green"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs font-bold text-promaroc-green truncate mb-1">
                        {conv.property?.title}
                      </div>
          ...
          <>
            <div className="p-4 border-b border-black/5 dark:border-white/5 bg-white dark:bg-[#0a0a0a] flex items-center gap-4 shrink-0">
              <div className="w-12 h-12 rounded-xl bg-black/5 dark:bg-white/5 overflow-hidden shrink-0">
                {(activeConversation?.property?.heroImage ||
                  newChatProperty?.heroImage) && (
                  <img
                    src={
                      activeConversation?.property?.heroImage ||
                      newChatProperty?.heroImage ||
              ...
              <div>
                <h3 className="font-bold font-sora line-clamp-1">
                  {activeConversation?.property?.title || newChatProperty?.title}
                </h3>
                <div className="text-xs font-medium text-black/50 dark:text-white/50 flex items-center gap-1">
                  <MapPin className="w-3 h-3" />
                  {activeConversation?.property?.location ||
                    newChatProperty?.location}
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-white dark:bg-[#0a0a0a] border-t border-black/5 dark:border-white/5 shrink-0">
              <form action={sendMessage} className="flex items-center gap-3">
                ...
                <button
                  type="submit"
                  className="w-12 h-12 bg-promaroc-green text-white rounded-full flex items-center justify-center hover:bg-[#0a2e29] transition-colors shrink-0 shadow-md"
                >
